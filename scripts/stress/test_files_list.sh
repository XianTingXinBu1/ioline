...
run_request() {
  local output http_code duration_ms
  output="$(curl -sS -o /dev/null -w '%{http_code} %{time_total}' "$BASE_URL/api/files/list?path=$TARGET_PATH")"
  http_code="${output%% *}"
  duration_ms="$(awk -v sec="${output##* }" 'BEGIN { printf "%d", sec * 1000 }')"
  printf '%s %s\n' "$http_code" "$duration_ms"
}

export BASE_URL TARGET_PATH
